...
public class Uzaklik {
	...
	public double minumumUzaklik() {
		...
		System.out.printf("Birbirine en yakýn iki nokta;%nÝlk nokta: %d",enYakinNoktalar[0][0]);
		System.out.printf(",%d ve ",enYakinNoktalar[0][1]);
		System.out.printf("Ýkinci nokta: %d",enYakinNoktalar[1][0]);
	...
	public void enYakinIkiNoktayiBul(int noktalar[][]) {
		...
		double enYakinÝkiNoktaArasýUzaklik = 10;
		...
		for (int i = 0; i < noktalar.length; i++) {
			...
			for (int j = i+1; j < noktalar.length; j++) {
				...
				if (ikiNoktaArasiUzaklik < enYakinÝkiNoktaArasýUzaklik) {
					...
					System.out.printf("En Yakýn Ýki Nokta Güncelendi%n");
				}
				System.out.printf("%d.Nokta ile %d.Nokta arasý fark: %.2f %n%n",i+1,j+1,ikiNoktaArasiUzaklik);
			}
		}
	}
